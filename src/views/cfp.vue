  ...
  <div>
    ...
    <div class="list border_t" v-for="(item,index) in lists" :key="index">
      ...
      <div class="usr border_b">
        <div class="usr-info">
          <p>乘客：{{item.NickName}}</p>
          <p>需求{{item.SCount}}个座位</p>
          <p>
            愿付
    ...
    <div class="no-result" v-else>
      <p>未找到符合条件的乘客~</p>
      <p>你可以去发布约乘信息</p>
      <van-button type="default" class="btn">发布约乘信息</van-button>
    </div>
  </div>
</template>
<script>
export default {
  ...
  created() {
    ...
    this.$post('http://10.102.144.75:8022/api/Schedu/GetSchedu',{"startLat":this.commonFnc.getQueryString("startLat"),"startLon":this.commonFnc.getQueryString("startLon"),"endLat":this.commonFnc.getQueryString("endLat"),"endLon":this.commonFnc.getQueryString("endLon"),"startTime":this.commonFnc.getQueryString("startTime"),"type":"0"}).then(data=>{
      ...
      _tc_bridge_util.show_tips_dialog({
        ...
        callback: data=> {
          if (data.tagname == "tag_click_right") {
              this.bridgeFnc.openNewUrl(window.location.origin+window.location.pathname+'#/orderdetail')
          }
        }
      });
    },
